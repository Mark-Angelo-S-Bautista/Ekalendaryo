// ==========================
// Search Events (optional - only if search exists)
// ==========================
const searchInput = document.getElementById("search");
const clearBtn = document.getElementById("clearSearch");
const eventList = document.getElementById("eventList");

if (searchInput && clearBtn && eventList) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (modal && feedbackList) {
        ...
        function loadFeedback(page = 1) {
            fetch(
                `/usermanagement/event/${currentEventId}/feedback?page=${page}`,
            )
                .then((res) => res.text())
                .then((html) => {
    ...
    function attachRemoveButtons() {
        document.querySelectorAll(".remove-report-btn").forEach((btn) => {
            btn.addEventListener("click", function () {
                ...
                fetch(`/usermanagement/events/${eventId}/remove-report`, {
                    method: "POST",
                    headers: {
